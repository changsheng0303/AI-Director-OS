...
圆桌诊断不是常规自检，而是一种重型、多视角、用户可见的诊断产品：它会召集4–6个不同立场的功能性评委，分别引用剧本证据，最后由主治医生整理分歧。它只诊断，不直接改稿，也不替用户作最终裁决。
...
| “从制片、编剧、导演、平台、观众角度审” | 是 | 圆桌多视角诊断 |
| “给我开圆桌会诊/找几个评委争论” | 是 | 圆桌诊断 |
...
1. 为新手增加一页“只选任务、不选Skill”的快速入口说明；
2. 为38个Skill建立自动生成的索引与许可证清单；
3. 为圆桌诊断增加精简模式，允许只召集3个最相关视角；
4. 用真实项目记录圆桌介入前后对返工率和剧本质量的影响；
5. 持续将真实生成失败回写到分镜和提示词校验。
