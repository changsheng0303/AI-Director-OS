---
name: micro-drama-creation
description: "创作 50–100 集微短剧或竖屏短剧，覆盖选题、角色、分集大纲、逐集剧本、爽点、钩子、付费卡点、出海、质量与合规审核。"
---
...
**强约定(2026-08):每个项目必须有自己的独立文件夹**——剧本、提示词、角色/场景资产、设定文档全部收拢在该项目目录下,严禁散落在共享目录(如 `{当前工作区}\剧本\`、`h3-prompts\` 根下)。新建项目时先创建 `{当前工作区}\<项目名>\` 项目根,再按以下结构组织:
...
> 迁移提示:存量项目(如爆衣味觉学园)文件散在 `{当前工作区}\剧本\` 与 `h3-prompts\` 下,后续整理时按上述结构收拢。
...
| dna-extraction.md | 对标作品 DNA 提取（用户点名对标时必读） | /start, /plan |
| 四视角顾问审稿.md | 定稿前四视角独立评估（编剧/制片/观众/平台） | /plan 定稿前 |
