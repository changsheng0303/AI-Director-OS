...
> - 用户提交剧本 → LLM 按模式召集 3 位精简评委或 4-6 位完整评委 → 评委按顺序独立陈述 → 主治医生归纳清单
...
当 `screenplay-master` 因“多视角深度审稿”自动路由到本模块，而用户没有明确指定人数时，默认使用**精简3视角**。用户明确说“完整圆桌”“全面会诊”“召集所有相关方”或手动指定4位以上评委时，使用完整模式。
...
精简模式保留证据、立场冲突和归纳机制，但压缩流程：

1. 自动选择3个互补功能位：
   - **结构/工艺**：资深类型片编剧或责任编辑；
   - **受众/市场**：核心受众、路人观众或平台采购；
   - **生产/呈现**：商业制片、演员经纪或影评人。
2. 评委名单在报告开头展示，但不暂停等待用户确认；
3. 每位评委控制在120–220字，仍必须引用剧本证据；
4. 主治医生归纳控制在300–500字；
5. 共识规则按3人调整：
   - 🔴 必改：3人独立指向同一问题；
   - 🟡 建议：2人独立指出；
   - ⚪ 可议：1人提出但证据成立；
   - 🟢 别动：3人一致认可；
6. 精简模式不自动加入极端评委；`精简 +极端`时增加1位极端视角，成为3+1模式。

精简模式适合完整初稿后的第一轮多视角深审、单集里程碑检查和迭代前问题收敛。它不用于一句灵感、纯错字、纯格式或每写一场后的自动自我批评。
...
**输入**：完整剧本、完整大纲或单集剧本
**触发**：`会诊 精简`、`圆桌精简`、`深度审稿 多视角`
**重点**：用结构、受众、生产三个互补视角收敛最值得改的事项
**输出长度**：评委陈述总计约500–900字＋归纳300–500字
**限制**：不替代六门常规自检，不直接改写剧本，不在每次生成后自动运行
...
| **精简会诊** | 会诊精简、圆桌精简、精简圆桌、三人会诊、3视角会诊、深度审稿多视角 |
