...
Write all six rewrite sections in Chinese (简体中文). Keep every format element unchanged — the six section field names, reference labels (`<Subject N>` / `<Picture N>` / `<Video N>` / `<Audio N>`), speaker IDs, camera-motion terms, task-type prefixes, and fixed relationship markers all stay in their fixed English forms. Preserve the original language only for dialogue and lyrics inside `<d>` and for text visibly present in the scene. The English examples in this guide show format structure only; the actual body text must be Chinese.
...
This section uses one short Chinese paragraph to summarize the target video and its reference relationships. It begins with a square-bracketed task-type prefix:
...
- Write the body in Chinese. Preserve the original language of dialogue, lyrics, and visible text.
- `[Shot 1]` marks the opening shot and has no timestamp. Later shots use `[Shot N] At MM:SS.mmm, ...` to mark cut times.
- Write camera movement using only the fixed English motion terms (motion type + optional amplitude + optional speed), embedded naturally within the current Chinese shot description.
...
| Style opening | Written after `[Shot 1]` | Established in one or two Chinese sentences before `[Shot 1]` |
...
For generation tasks, `detailed_description` is normally 350-500 Chinese characters (汉字字数). Dialogue-dense content prioritizes fitting the complete spoken timeline rather than mechanically reaching a character count. Video-editing descriptions scale with the complexity of the source video and do not have to follow the generation-task range. A single shot does not automatically justify a shorter description; distribute detail across multiple shots according to their information load.
