...
> **Output language:** Write the entire prompt body in Chinese (简体中文). Keep every format element unchanged — section field names, shot/cut markers (`[Shot 1]`, `[Shot N] At MM:SS.mmm`), camera-motion terms, speaker IDs, `<d>` tags, and instruction templates all stay in their fixed English forms. The English examples below show format structure only; the actual body text must be Chinese. Preserve the original language only inside `<d>` dialogue blocks and for on-screen text.
...
Camera motion should be written as a natural action within the shot using only the fixed English motion terms (motion type + optional amplitude + optional speed), embedded into the Chinese body text rather than stacked as separate labels at the end of a sentence:
